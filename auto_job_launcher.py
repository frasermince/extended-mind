...
def compute_tasks_num_per_job(task_max_time, max_time_per_job, num_parallel_tasks=1):
    ...
    if task_seconds > job_seconds:
        print(
            f"ERROR: Task time with buffer ({task_seconds}s) exceeds job time ({job_seconds}s). No tasks can fit in this job."
        )
        return 0
    ...
    if num_parallel_tasks > 1:
        print(
            f"Parallel execution: {num_parallel_tasks} tasks per round, {num_rounds} rounds = {total_tasks} total tasks per job"
        )
    else:
...
def generate_script(
    ...
    for a_task_conf in task_confs:
        ...
        for key, value in a_task_conf.items():
            if (
                key == "exp_name"
                or key == "wandb_api_key"
                or key == "run_folder"
                or key == "parquet_folder"
                or key == "agent_name"
            ):
                continue
        ...
        run_folder_param = (
            f"run_folder={shlex.quote(run_folder)} "
            if (run_folder and agent_name != "linear_qlearning")
            else ""
        )
        parquet_folder_param = (
            f"parquet_folder={shlex.quote(parquet_folder)} " if parquet_folder else ""
        )
        if wandb_api_key == "":
            print(
                "Warning : launching without wandb tracking. Either the prgram does not have it or it has it and you have not specified the api key."
            )
            commands.append(
                f"uv run python src/{script_name}.py {run_folder_param}{parquet_folder_param}{script_params}"
            )
        else:
            commands.append(
                f"uv run python src/{script_name}.py wandb_api_key={shlex.quote(wandb_api_key)} {run_folder_param}{parquet_folder_param}{script_params}"
            )

    script_dir = os.path.dirname(os.path.abspath(__file__))
    script = generate_job_script_from_template(
        num_parallel_tasks,
        threads_per_task,
        cluster_conf["mem_per_cpu"],
        max_job_time,
        cluster_conf["account"],
        commands,
        script_dir,
        gpus_zero,
    )
...
def generate_task_configs_per_job(
    seeds,
    ...
    num_tasks_per_job = compute_tasks_num_per_job(
        max_task_time, max_job_time, num_parallel_tasks
    )
    print(f"Number of tasks per job: {num_tasks_per_job}")

    if num_tasks_per_job == 0:
        raise ValueError(
            f"Cannot fit any tasks in job with time limit {max_job_time}. Task time with buffer exceeds job time."
        )
    ...
    for tc in task_confs:
        ...
        print(
            f">>>>> Run path: {run_path}"
        )  # TODO: comment this block if needed ... it can be too slow
...
def check_if_file_exists(file_path):
    ...
    print(
        f">>>>> File exists: {os.path.exists(metrics_path) or os.path.exists(metrics_optimal_path)}"
    )
...
def construct_run_path(tc_original, run_folder, parquet_folder):
    ...
    if agent_name == "main_dqn":
        ...
        if path_mode == "VISITED_CELLS":
            ...
            nonstationary_path_inclusion_pixels = tc[
                "nonstationary_path_inclusion_pixels"
            ]

            decay_chance_str = f"{float(nonstationary_path_decay_chance):.2f}".rstrip(
                "0"
            ).rstrip(".")
...
def main():
    ...
    task_confs_per_job = generate_task_configs_per_job(
        list(range(seed_offset, seed_offset + args.num_seeds)),
        hyperparam_sweep_conf,
        hyperparam_default_conf,
        args.max_task_time,
        args.max_job_time,
        args.run_folder,
        args.parquet_folder,
    ...
    for one_job_task_confs in task_confs_per_job:
        ...
        script = generate_script(
            task_confs=one_job_task_confs,
            cluster_conf=cluster_conf,
            max_job_time=args.max_job_time,
