...
def _plot_grouped_bars_path_vs_pathless_by_capacity(
    ...
    yticks = np.linspace(0, 12500, 6)
    ax.set_yticks(yticks)
    ax.set_yticklabels(yticks)
    ax.set_ylim(0, 12500)
    ax.set_ylabel(ylabel, fontsize=26)
    ax.set_title(title_with_config, fontsize=26)
    ax.grid(False)
...
def nonstationary_main():
    ...
    nonstationary_linear = "/Users/frasermince/Programming/hidden_llava/current_processed/nonstationary_linear_aggregation_progress.jsonl"
...
def all_paths_main():
    ...
    # just_landmarks = [("Landmarks", denser_landmarks_df)]
    path_tuples = [
        ("No Path", no_path_df),
