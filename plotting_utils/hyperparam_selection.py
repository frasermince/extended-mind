...
def aggregate_runs_linear_nonstationary_duckdb(
    ...
    *,
    ...
    print(
        "Discovering unique (learning_rate, agent_pixel_view_edge_dim, decay_pixels) combinations..."
    )
    combo_query = f"""
        SELECT DISTINCT 
            CAST(step_size AS DOUBLE) AS learning_rate,
            CAST(agent_pixel_view_edge_dim AS INTEGER) AS agent_pixel_view_edge_dim,
            CAST(nonstationary_path_decay_pixels AS INTEGER) AS decay_pixels
        FROM read_parquet('{glob_path}')
        WHERE metric = 'reward_per_timestep'{where_extra_sql}
        ORDER BY learning_rate, agent_pixel_view_edge_dim, decay_pixels
    """
    combos = con.execute(combo_query).fetchall()
    print(f"Found {len(combos)} unique (lr, dim, decay_pixels) combinations")
    con.close()
    ...
    # Use a permanent JSONL file in the project directory for resumability
    jsonl_path = os.path.join(
        os.path.dirname(__file__), "linear_aggregation_progress.jsonl"
    )
    print(f"Using results file: {jsonl_path}")
    ...
    lr_pattern = re.compile(r'"learning_rate":\s*([0-9.eE+-]+)')
    dim_pattern = re.compile(r'"agent_pixel_view_edge_dim":\s*(\d+)')
    decay_pattern = re.compile(r'"nonstationary_path_decay_pixels":\s*(\d+)')
        ...
        with open(jsonl_path, "r", encoding="utf-8") as f:
            for line in f:
                try:
                    lr_match = lr_pattern.search(line)
                    dim_match = dim_pattern.search(line)
                    decay_match = decay_pattern.search(line)
                    if lr_match and dim_match and decay_match:
                        lr = float(lr_match.group(1))
                        dim = int(dim_match.group(1))
                        decay = int(decay_match.group(1))
                        processed_combos.add((lr, dim, decay))
                except (ValueError, AttributeError):
                    pass  # Skip malformed lines
        print(f"Found {len(processed_combos)} already-processed combinations")
        gc.collect()  # Free any memory from file reading
    ...
    with open(jsonl_path, "a", encoding="utf-8") as jsonl_file:
        for combo_idx, (lr, dim, decay_pixels) in enumerate(combos):
            # Debug: skip to specific combo if requested
            if combo_idx < start_from_combo:
                print(
                    f"\nDebug skip {combo_idx + 1}/{len(combos)}: lr={lr}, dim={dim}, decay={decay_pixels}"
                )
                continue

            # Skip if already processed (now includes decay_pixels)
            if (lr, dim, decay_pixels) in processed_combos:
                print(
                    f"\nSkipping {combo_idx + 1}/{len(combos)}: lr={lr}, dim={dim}, decay={decay_pixels} (already done)"
                )
                continue

            # Extra cleanup before processing - important after skipping many combos
            print(
                f"\n--- Starting combo {combo_idx + 1}/{len(combos)}: lr={lr}, dim={dim}, decay={decay_pixels} ---",
                flush=True,
            )
            gc.collect()
            print_memory()
        ...
        WITH base AS (
            SELECT
                CAST(agent_pixel_view_edge_dim AS INTEGER) AS agent_pixel_view_edge_dim,
                CAST(nonstationary_path_decay_pixels AS INTEGER) AS nonstationary_path_decay_pixels,
                CAST(nonstationary_path_decay_chance AS DOUBLE) AS nonstationary_path_decay_chance,
                CAST(nonstationary_path_inclusion_pixels AS INTEGER) AS nonstationary_path_inclusion_pixels,
                CAST(step_size AS DOUBLE) AS learning_rate,
                COALESCE(CAST(optimal_path_available AS BOOLEAN), FALSE) AS optimal_path,
                CAST(seed AS INTEGER) AS seed,
                CAST(step AS BIGINT) AS step,
            ...
            FROM read_parquet('{glob_path}')
                WHERE metric = 'reward_per_timestep'
                  AND CAST(step_size AS DOUBLE) = {lr}
                  AND CAST(agent_pixel_view_edge_dim AS INTEGER) = {dim}
                  AND CAST(nonstationary_path_decay_pixels AS INTEGER) = {decay_pixels}
                  {where_extra_sql}
        ),
        per_seed_auc AS (
            SELECT
...
if __name__ == "__main__":
    if False:
        ...
        use_parquet = True
        if use_parquet:
            outputs_root = "/Users/frasermince/Programming/hidden_llava/current_processed/parquet_runs_linear_beyond_basic_paths"
            ...
            if False:
                ...
                try:
                    ...
                    #     step_subsample=10,
                    # )
                    outputs_root = "/Users/frasermince/Programming/hidden_llava/current_processed/linear_landmarks/agent_name_linear_qlearning/path_mode_LANDMARKS"
                    aggregate_runs_linear_duckdb(
                        outputs_root=outputs_root,
                        jsonl_path="/Users/frasermince/Programming/hidden_llava/current_processed/path_mode_LANDMARKS_aggregation_progress.jsonl",
                        recursive_glob="**/*.parquet",
                        step_subsample=10,
                    )
