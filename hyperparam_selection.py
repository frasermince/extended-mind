    ...
    for run_key, results in sorted_results:
        ...
        for i in range(len(items[0]["average_reward_per_timestep"])):
            if i % 10 == 0:
                ...
                # Shaped (steps, num_seeds)
                all_avg_episodic_reward.append(values_at_i)

        standard_error_reward_curve = np.std(
            all_avg_episodic_reward, ddof=1, axis=1
        ) / np.sqrt(len(all_avg_episodic_reward[0]))
        # shaped (num_seeds, steps)
        per_seed_avg_episodic_reward = np.swapaxes(all_avg_episodic_reward, 0, 1)
        total_rewards = []
        for curve in per_seed_avg_episodic_reward:
            total_rewards.append(curve[-1] * curve.shape[0] * 10)

        per_seed_aucs = []
        for seed_curve in per_seed_avg_episodic_reward:
            per_seed_aucs.append(np.trapezoid(seed_curve, dx=1).item())
        average_reward_area_under_curve = np.mean(per_seed_aucs)

        per_seed_auc_standard_error = np.std(per_seed_aucs, ddof=1, axis=0) / np.sqrt(
            len(per_seed_aucs)
        )
        ...
        record = {
            ...
            "average_reward_curve": list(all_avg_episodic_reward),
            "average_reward_curve_standard_error": list(standard_error_reward_curve),
            "per_seed_aucs": per_seed_aucs,
            "per_seed_auc_standard_error": per_seed_auc_standard_error,
