...
def agent_observation_transform(
    img_obs: jnp.ndarray, agent_pixel_view_edge_dim: int, obs_dims: tuple
):
...
def closed_form_q_update(
    ...
    q_next_max = jnp.where(
        done, 0.0, jnp.max(q_next_values)
    )  # if done, q_next_target is 0, otherwise it is the max q value of the next state
    td_error = (r + discount * q_next_max) - q_curr  # the classic td error

    # Compute gradients for weight and bias
    weight_gradient = jnp.outer(
        x, td_error
    )  # outer product gives us the gradient matrix
...
class NumpyQLearningAgent:
    ...
    def select_action(self, x: jnp.ndarray, key: jnp.ndarray, global_step: int):
        ...
        q_values = self.linear_q_network.apply(
            x
        )  # get the q values for the state / observation
        greedy_action = jnp.argmax(
            q_values
        )  # greedy action is one with the max q value
        ...
        p = jax.random.uniform(
            key, shape=(), minval=0.0, maxval=1.0
        )  # probability of choosing a random action
        action = jnp.where(
            p < epsilon, random_action, greedy_action
        )  # choose a random action with probability epsilon, otherwise choose the greedy action
    ...
    def update_q_values(
        ...
        self.linear_q_network.params = {"weight": new_weight, "bias": new_bias}

        return loss

    def _agent_observation_transform(self, img_obs: jnp.ndarray):
        return agent_observation_transform(
            img_obs, self.agent_pixel_view_edge_dim, self.obs_dims
        )
...
def q_update(
    ...
    def loss_fn(params: jnp.ndarray):
        ...
        q_curr = q_values[a]  # select the q value for the action taken
        q_next_max = jnp.where(
            done, 0.0, jnp.max(agent_train_state.apply_fn(params, x_next))
        )  # if done, q_next_target is 0, otherwise it is the max q value of the next state
        td_error = (
            jax.lax.stop_gradient(r + discount * q_next_max) - q_curr
        )  # the classic td error
    ...
    new_agent_train_state = agent_train_state.apply_gradients(
        grads=grads
    )  # update the agent q values based on gradients
...
class FlaxQLearningAgent:
    def __init__(
        ...
        linear_q_network_params = linear_q_network.init(
            jax.random.PRNGKey(seed), jnp.zeros((self.num_states,))
        )
...
def compare_numpy_vs_flax_losses(cfg, num_comparisons: int = 5):
    ...
    num_states = (
        cfg.training.agent_pixel_view_edge_dim
        * cfg.training.agent_pixel_view_edge_dim
        * image_shape[2]
    )
    shared_weight = nn.initializers.lecun_normal()(
        shared_key, (num_states, env.action_space.n)
    )
    shared_bias = jnp.zeros(env.action_space.n)

    numpy_agent.linear_q_network.params = {"weight": shared_weight, "bias": shared_bias}
    ...
    for _ in range(num_comparisons):
        ...
        flax_agent.train_state = flax_agent.train_state.replace(
            params=original_flax_params.copy()
        )
        ...
        numpy_loss = numpy_agent.update_q_values(
            obs["image"], int(a), r, obs_next["image"], done, cfg.training.discount
        )
        flax_loss = flax_agent.update_q_values(
            obs["image"], int(a), r, obs_next["image"], done, cfg.training.discount
        )
...
def train(
    ...
    print(
        "Optimal policy expected steps: ",
        abs(unwrapped_env.agent_pos[0] - unwrapped_env.goal_position[0])
        + abs(unwrapped_env.agent_pos[1] - unwrapped_env.goal_position[1]),
    )
...
def eval_env(
    cfg, envs, path_list, model_params, action_mode, actions_list, seed, video_dir=None
):
    ...
    if "weight" in model_params and "bias" in model_params:
        ...
        def apply_q_network(x):
            x_transformed = agent_observation_transform(
                x, agent_pixel_view_edge_dim, image_shape
            )
        ...
        raise ValueError(
            f"Unknown model_params format. Expected 'weight'/'bias' or 'params' keys, got: {list(model_params.keys())}"
        )
...
@hydra.main(
    config_path="../", config_name="linear_qlearning_config.yaml", version_base=None
)
def main(cfg):
    start_time = time.time()
